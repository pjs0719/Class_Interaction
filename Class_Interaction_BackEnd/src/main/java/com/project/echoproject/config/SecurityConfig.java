...
import com.project.echoproject.jwt.LogOut;
    ...
    public SecurityFilterChain filterChain(HttpSecurity http) throws Exception {
        ...
        http
                .authorizeHttpRequests((auth) -> auth
                        .requestMatchers("/login", "/", "/join", "/checkEmail","/reissue", "/classroomEnter/**", "/watch/**","/fcm"
                        ).permitAll() //모든 권한
                        .requestMatchers("/instructor/").hasRole("instructor") //교수계정만
                        .anyRequest().authenticated());

        http.addFilterBefore(new JWTFilter(jwtUtil), LoginFilter.class);
        http.addFilterBefore(new LogOut(jwtUtil,refreshService), LogoutFilter.class);
        //필터 추가 LoginFilter()는 인자를 받음 (AuthenticationManager() 메소드에 authenticationConfiguration 객체를 넣어야 함) 따라서 등록 필요
        http
                .addFilterAt(new LoginFilter(authenticationManager(authenticationConfiguration), jwtUtil, userRepository, classroomRepository, studentRepository, instructorRepository, enrollmentRepository,refreshService), UsernamePasswordAuthenticationFilter.class);
